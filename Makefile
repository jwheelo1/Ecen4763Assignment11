...
clean:
	...
	rm -rf cryptography/rsa/__pycache__ # remove compiled python directory
	rm -rf test/rsa/__pycache__ # remove compiled python directory
...
format:
	...
	autopep8 --in-place --aggressive --experimental --max-line-length $(MAX_LINE) --recursive cryptography/
	...
	docformatter --in-place --wrap-summaries $(MAX_LINE) --wrap-descriptions $(MAX_LINE) \
		--pre-summary-newline --recursive cryptography/
	docformatter --in-place --wrap-summaries $(MAX_LINE) --wrap-descriptions $(MAX_LINE) \
		--pre-summary-newline --recursive test/

style: .python-check
	@echo "Check if 'cryptography' folder matches PEP-8 and PEP-257 guides"
	pydocstyle cryptography/
	pycodestyle cryptography/ --max-line-length=$(MAX_LINE)
...
test: clean .python-check
	coverage run --source=cryptography --omit=*"__init__.py" -m unittest -v
